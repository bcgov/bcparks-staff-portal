...
function getPrimaryContactEmail(staffContacts = []) {
  ...
  for (const fragment of jobTitleSubstringPriority) {
    primaryContact = activeStaffContacts.find((contact) =>
      contact.title?.toLowerCase().includes(fragment),
    );
    if (primaryContact) break;
  }

  primaryContact = primaryContact || activeStaffContacts[0] || null;
